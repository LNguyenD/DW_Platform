...
/****** Object:  View [views].[HEM_RTW_Agency_Group_Compares_To_Same_Time_Last_Year_Current_View]    Script Date: 01/13/2016 14:43:33 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[views].[hem_rtw_agency_group_compares_to_same_time_last_year_current_view]'))
DROP VIEW [views].[hem_rtw_agency_group_compares_to_same_time_last_year_current_view]
GO


CREATE VIEW views.[hem_rtw_agency_group_compares_to_same_time_last_year_current_view] 
AS
	WITH temp AS 
	(
		select * 
	...
	select Month_period=case when DATEDIFF(MM, Remuneration_Start, Remuneration_End) = 0
			...
			,[Target] = sum(LT)/nullif(sum(WGT),0)*100/nullif(udfs.dashboard_hem_rtw_gettargetandbase_udf(Remuneration_End,'target','','Hospitality',NULL,Measure),0)
	from views.RTW_view uv
	where  uv.Remuneration_End = (SELECT max(Remuneration_End) FROM  views.RTW_view)
		   and  DATEDIFF(MM, Remuneration_Start, Remuneration_End) in (0,2,5,11)
	...
	select Month_period=case when DATEDIFF(MM, Remuneration_Start, Remuneration_End) = 0
			...
			,sum(LT) as LT,sum(WGT) as WGT
			,sum(LT)/nullif(sum(WGT),0) as AVGDURN 
			,[Target] = sum(LT)/nullif(sum(WGT),0)*100/nullif(udfs.dashboard_hem_rtw_gettargetandbase_udf(Remuneration_End,'target','portfolio',rtrim(Portfolio),NULL,Measure),0)
	from views.RTW_view uv
	where  uv.Remuneration_End = (SELECT max(Remuneration_End) FROM  views.RTW_view)
	...
	select Month_period=case when DATEDIFF(MM, Remuneration_Start, Remuneration_End) = 0
			...
			,sum(LT) as LT,sum(WGT) as WGT
			,sum(LT)/nullif(sum(WGT),0) as AVGDURN 
			,[Target] = sum(LT)/nullif(sum(WGT),0)*100/nullif(udfs.dashboard_hem_rtw_gettargetandbase_udf(Remuneration_End,'target','portfolio','Hotel',NULL,Measure),0)
	from views.RTW_view uv
	where  uv.Remuneration_End = (SELECT max(Remuneration_End) FROM  views.RTW_view)
		   and  DATEDIFF(MM, Remuneration_Start, Remuneration_End) in (0,2,5,11) and rtrim(Portfolio) IS NOT NULL
	...
	select Month_period=case when DATEDIFF(MM, Remuneration_Start, Remuneration_End) = 0
			...
			,sum(LT) as LT,sum(WGT) as WGT
			,sum(LT)/nullif(sum(WGT),0) as AVGDURN 
			,[Target] = sum(LT)/nullif(sum(WGT),0)*100/nullif(udfs.dashboard_hem_rtw_gettargetandbase_udf(Remuneration_End,'target','group',udfs.hem_getgroup_byteam_udf(Team),NULL,Measure),0)
	from views.RTW_view uv
	where  uv.Remuneration_End = (SELECT max(Remuneration_End) FROM  views.RTW_view)
		   and  DATEDIFF(MM, Remuneration_Start, Remuneration_End) in (0,2,5,11)
	...
	select Month_period=case when DATEDIFF(MM, Remuneration_Start, Remuneration_End) = 0
			...
			,sum(LT) as LT,sum(WGT) as WGT
			,sum(LT)/nullif(sum(WGT),0) as AVGDURN 
			,[Target] = sum(LT)/nullif(sum(WGT),0)*100/nullif(udfs.dashboard_hem_rtw_gettargetandbase_udf(Remuneration_End,'target','account_manager',rtrim([Account_Manager]),NULL,Measure),0)
	from views.RTW_view uv
	where  uv.Remuneration_End = (SELECT max(Remuneration_End) FROM  views.RTW_view)
		   and  DATEDIFF(MM, Remuneration_Start, Remuneration_End) in (0,2,5,11)
		   and rtrim([Account_Manager]) is not null
