IF OBJECT_ID('views.hem_rtw_agency_group_rolling_month_12') IS NOT NULL
	DROP VIEW views.hem_rtw_agency_group_rolling_month_12
GO
CREATE VIEW views.hem_rtw_agency_group_rolling_month_12
AS
	SELECT    EmployerSize_Group = udfs.tmf_getgroup_byteam_udf(Team)
			  ...
			  ,[Target] = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'target','group',udfs.tmf_getgroup_byteam_udf(Team),NULL,uv.Measure)									
			  ,Base = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'base','group',udfs.tmf_getgroup_byteam_udf(Team),NULL,uv.Measure)
	...
	SELECT     EmployerSize_Group = rtrim(uv.Portfolio)
			  ...
			  ,WGT = SUM(uv.WGT)
			  ,AVGDURN = SUM(uv.LT) / nullif(SUM(uv.WGT),0)
			  ,[Target] = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'target','portfolio',uv.Portfolio,NULL,uv.Measure)									
			  ,Base = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'base','portfolio',uv.Portfolio,NULL,uv.Measure)					 
	FROM         views.RTW_view uv 
	WHERE	  DATEDIFF(MM, uv.Remuneration_Start, uv.Remuneration_End) =11 
				and uv.Remuneration_End between DATEADD(DAY, -1, DATEADD(M, -23 + DATEDIFF(M, 0, (SELECT max(Remuneration_End) FROM  views.RTW_view)), 0)) + '23:59' and (SELECT max(Remuneration_End) FROM  views.RTW_view)
	...
	SELECT     EmployerSize_Group = 'Hotel'
			  ...
			  ,WGT = SUM(uv.WGT)
			  ,AVGDURN = SUM(uv.LT) / nullif(SUM(uv.WGT),0)
			  ,[Target] = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'target','portfolio','Hotel',NULL,uv.Measure)									
			  ,Base = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'base','portfolio', 'Hotel',NULL,uv.Measure)					 
	FROM         views.RTW_view uv 
	WHERE	  DATEDIFF(MM, uv.Remuneration_Start, uv.Remuneration_End) =11 
				and uv.Remuneration_End between DATEADD(DAY, -1, DATEADD(M, -23 + DATEDIFF(M, 0, (SELECT max(Remuneration_End) FROM  views.RTW_view)), 0)) + '23:59' and (SELECT max(Remuneration_End) FROM  views.RTW_view)
				and uv.Portfolio is not null
	...
	SELECT     EmployerSize_Group = rtrim(uv.Account_Manager)
			  ...
			  ,WGT = SUM(uv.WGT)
			  ,AVGDURN = SUM(uv.LT) / nullif(SUM(uv.WGT),0)
			  ,[Target] = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'target','account_manager',uv.Account_Manager,NULL,uv.Measure)									
			  ,Base = udfs.tmf_rtw_gettargetandbase(uv.Remuneration_End,'base','account_manager',uv.Account_Manager,NULL,uv.Measure)					 
	FROM         views.RTW_view uv 
	WHERE	  DATEDIFF(MM, uv.Remuneration_Start, uv.Remuneration_End) =11 
				and uv.Remuneration_End between DATEADD(DAY, -1, DATEADD(M, -23 + DATEDIFF(M, 0, (SELECT max(Remuneration_End) FROM  views.RTW_view)), 0)) + '23:59' and (SELECT max(Remuneration_End) FROM  views.RTW_view)
	...
	SELECT     EmployerSize_Group ='Hospitality'
				...
				,[Target] = udfs.tmf_rtw_gettargetandbase(t.Remuneration_End,'target','group','Hospitality',NULL,t.Measure)
				,Base = udfs.tmf_rtw_gettargetandbase(t.Remuneration_End,'base','group','Hospitality',NULL,t.Measure)
	...
	SELECT     EmployerSize_Group= 'Hospitality'
				...
				,WGT =SUM(t.WGT)  
				,AVGDURN =SUM(LT) / nullif(SUM(WGT),0)  			
				,[Target] = udfs.tmf_rtw_gettargetandbase(t.Remuneration_End,'target','portfolio','Hospitality',NULL,t.Measure)
				,Base = udfs.tmf_rtw_gettargetandbase(t.Remuneration_End,'base','portfolio','Hospitality',NULL,t.Measure)
	...
	SELECT     EmployerSize_Group= 'Hospitality'
				...
				,WGT =SUM(t.WGT)  
				,AVGDURN =SUM(LT) / nullif(SUM(WGT),0)  			
				,[Target] = udfs.tmf_rtw_gettargetandbase(t.Remuneration_End,'target','account_manager','Hospitality',NULL,t.Measure)
				,Base = udfs.tmf_rtw_gettargetandbase(t.Remuneration_End,'base','account_manager','Hospitality',NULL,t.Measure)
