IF OBJECT_ID('views.hem_rtw_agency_group_compares_to_same_time_last_year_current_view') IS NOT NULL
	DROP VIEW views.hem_rtw_agency_group_compares_to_same_time_last_year_current_view
GO
...
AS
	SELECT    EmployerSize_Group = udfs.emi_getgroup_byteam_udf(Team)
			  ...
			  ,[Target] = udfs.dashboard_emi_rtw_gettargetandbase_udf(uv.Remuneration_End,'target','group',udfs.emi_getgroup_byteam_udf(Team),NULL,uv.Measure)									
			  ,Base = udfs.dashboard_emi_rtw_gettargetandbase_udf(uv.Remuneration_End,'base','group',udfs.emi_getgroup_byteam_udf(Team),NULL,uv.Measure)
	...
	SELECT     EmployerSize_Group = rtrim(uv.EMPL_SIZE)
			  ...
			  ,WGT = SUM(uv.WGT)
			  ,AVGDURN = SUM(uv.LT) / nullif(SUM(uv.WGT),0)
			  ,[Target] = udfs.dashboard_emi_rtw_gettargetandbase_udf(uv.Remuneration_End,'target','employer_size',uv.EMPL_SIZE,NULL,uv.Measure)									
			  ,Base = udfs.dashboard_emi_rtw_gettargetandbase_udf(uv.Remuneration_End,'base','employer_size',uv.EMPL_SIZE,NULL,uv.Measure)					 
	FROM         views.RTW_view uv 
	WHERE	  DATEDIFF(MM, uv.Remuneration_Start, uv.Remuneration_End) =11 
				and uv.Remuneration_End between DATEADD(DAY, -1, DATEADD(M, -23 + DATEDIFF(M, 0, (SELECT max(Remuneration_End) FROM  views.RTW_view)), 0)) + '23:59' and (SELECT max(Remuneration_End) FROM  views.RTW_view)
	...
	SELECT     EmployerSize_Group = rtrim(uv.Account_Manager)
			  ...
			  ,WGT = SUM(uv.WGT)
			  ,AVGDURN = SUM(uv.LT) / nullif(SUM(uv.WGT),0)
			  ,[Target] = udfs.dashboard_emi_rtw_gettargetandbase_udf(uv.Remuneration_End,'target','account_manager',uv.Account_Manager,NULL,uv.Measure)									
			  ,Base = udfs.dashboard_emi_rtw_gettargetandbase_udf(uv.Remuneration_End,'base','account_manager',uv.Account_Manager,NULL,uv.Measure)					 
	FROM         views.RTW_view uv 
	WHERE	  DATEDIFF(MM, uv.Remuneration_Start, uv.Remuneration_End) =11 
				and uv.Remuneration_End between DATEADD(DAY, -1, DATEADD(M, -23 + DATEDIFF(M, 0, (SELECT max(Remuneration_End) FROM  views.RTW_view)), 0)) + '23:59' and (SELECT max(Remuneration_End) FROM  views.RTW_view)
	...
	SELECT     EmployerSize_Group ='WCNSW'
				...
				,[Target] = udfs.dashboard_emi_rtw_gettargetandbase_udf(t.Remuneration_End,'target','group','EML',NULL,t.Measure)
				,Base = udfs.dashboard_emi_rtw_gettargetandbase_udf(t.Remuneration_End,'base','group','EML',NULL,t.Measure)
	...
	SELECT     EmployerSize_Group= 'WCNSW'
				...
				,WGT =SUM(t.WGT)  
				,AVGDURN =SUM(LT) / nullif(SUM(WGT),0) 			
				,[Target] = udfs.dashboard_emi_rtw_gettargetandbase_udf(t.Remuneration_End,'target','employer_size','EML',NULL,t.Measure)
				,Base = udfs.dashboard_emi_rtw_gettargetandbase_udf(t.Remuneration_End,'base','employer_size','EML',NULL,t.Measure)
	...
	SELECT     EmployerSize_Group= 'WCNSW'
				...
				,WGT =SUM(t.WGT)  
				,AVGDURN =SUM(LT) / nullif(SUM(WGT),0) 			
				,[Target] = udfs.dashboard_emi_rtw_gettargetandbase_udf(t.Remuneration_End,'target','account_manager','EML',NULL,t.Measure)
				,Base = udfs.dashboard_emi_rtw_gettargetandbase_udf(t.Remuneration_End,'base','account_manager','EML',NULL,t.Measure)
