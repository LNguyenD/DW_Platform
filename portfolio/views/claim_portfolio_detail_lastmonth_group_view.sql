...
AS
	...
	claim_new_all AS
	(
		SELECT Value = [Group], SubValue = Team, SubSubValue = Claims_Officer_Name, [System], Claim_No, Date_Of_Injury, Is_Time_Lost, Claim_Closed_Flag,
	...
	(
		SELECT	CAST(tmp.Value AS VARCHAR(256)) AS [Value]
				,CAST(tmp.SubValue AS VARCHAR(256)) AS [SubValue]
				,CAST(tmp.SubSubValue AS VARCHAR(256)) AS [SubSubValue]
		...
		(
			...
			cross join (select distinct Value, SubValue, SubSubValue, [System]
						from claim_all
						where Value <> '' and SubValue <> '' and SubSubValue <> '') as tmp_value
		) as tmp
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as ffsd_at_work_15_less, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek <= 15
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp1 ON tmp1.[System] = tmp.[System] and tmp1.Value = tmp.Value 
			and tmp1.SubValue = tmp.SubValue 
			and tmp1.SubSubValue = tmp.SubSubValue 
			and tmp1.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as ffsd_at_work_15_more, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek > 15
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp2 ON tmp2.[System] = tmp.[System] and tmp2.Value = tmp.Value 
			and tmp2.SubValue = tmp.SubValue
			and tmp2.SubSubValue = tmp.SubSubValue
			and tmp2.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as ffsd_not_at_work, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'SID' and Is_Working = 0
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp3 ON tmp3.[System] = tmp.[System] and tmp3.Value = tmp.Value 
			and tmp3.SubValue = tmp.SubValue
			and tmp3.SubSubValue = tmp.SubSubValue
			and tmp3.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as pid, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'PID'
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp4 ON tmp4.[System] = tmp.[System] and tmp4.Value = tmp.Value 
			and tmp4.SubValue = tmp.SubValue 
			and tmp4.SubSubValue = tmp.SubSubValue
			and tmp4.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as totally_unfit, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'TU'
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp5 ON tmp5.[System] = tmp.[System] and tmp5.Value = tmp.Value 
			and tmp5.SubValue = tmp.SubValue 
			and tmp5.SubSubValue = tmp.SubSubValue
			and tmp5.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as therapy_treat, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all
			...
			and tmp6.SubValue = tmp.SubValue 
			and tmp6.SubSubValue = tmp.SubSubValue
			and tmp6.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as d_d, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Is_D_D = 1
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp7 ON tmp7.[System] = tmp.[System] and tmp7.Value = tmp.Value 
			and tmp7.SubValue = tmp.SubValue 
			and tmp7.SubSubValue = tmp.SubSubValue
			and tmp7.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as med_only, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where Is_Medical_Only = 1
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp8 ON tmp8.[System] = tmp.[System] and tmp8.Value = tmp.Value 
			and tmp8.SubValue = tmp.SubValue 
			and tmp8.SubSubValue = tmp.SubSubValue
			and tmp8.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as lum_sum_in, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			...
			and tmp9.SubValue = tmp.SubValue 
			and tmp9.SubSubValue = tmp.SubSubValue
			and tmp9.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as ncmm_this_week, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			...
			and tmp10.SubValue = tmp.SubValue 
			and tmp10.SubSubValue = tmp.SubSubValue
			and tmp10.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as ncmm_next_week, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all 
			where NCMM_Actions_Next_Week <> ''
				and NCMM_Prepare_Action_Due BETWEEN DATEADD(week, 1, '2016-01-11 23:59') AND DATEADD(week, 3, '2016-01-11 23:59')
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp11 ON tmp11.[System] = tmp.[System] and tmp11.Value = tmp.Value 
			and tmp11.SubValue = tmp.SubValue
			and tmp11.SubSubValue = tmp.SubSubValue
			and tmp11.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as overall, [System], Value, SubValue, SubSubValue, claim_type
			from claim_all
			group by [System], Value, SubValue, SubSubValue, claim_type
		) tmp12 ON tmp11.[System] = tmp.[System] and tmp12.Value = tmp.Value 
			and tmp12.SubValue = tmp.SubValue
			and tmp12.SubSubValue = tmp.SubSubValue
	...
	SELECT	Value,
			...
			[Sum] = (select (case when tmp_port_type.PORT_Type = 'ffsd_at_work_15_less'
					...
					where tmp_total_2.Value = tmp_total.Value
						and tmp_total_2.SubValue = tmp_total.SubValue
						and tmp_total_2.SubSubValue = tmp_total.SubSubValue
