IF OBJECT_ID('views.claim_portfolio_detail_lastmonth_account_manager_view') IS NOT NULL
	DROP VIEW views.claim_portfolio_detail_lastmonth_account_manager_view
GO
CREATE VIEW views.claim_portfolio_detail_lastmonth_account_manager_view
AS
	...
	claim_new_all AS
	(
		SELECT Value = Account_Manager, SubValue = EMPL_SIZE, [System], Claim_No, Date_Of_Injury, Is_Time_Lost, Claim_Closed_Flag,
	...
	(
		...
		(
			...
			cross join (select distinct Value, SubValue, [System]
						from claim_all
						where Value <> '' and SubValue <> '') as tmp_value
		) as tmp
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as ffsd_at_work_15_less, [System], Value, SubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek <= 15
			group by [System], Value, SubValue, claim_type
		) tmp1 ON tmp1.[System] = tmp.[System] and tmp1.Value = tmp.Value 
			and tmp1.SubValue = tmp.SubValue and tmp1.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as ffsd_at_work_15_more, [System], Value, SubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'SID' and Is_Working = 1 and HoursPerWeek > 15
			group by [System], Value, SubValue, claim_type
		) tmp2 ON tmp2.[System] = tmp.[System] and tmp2.Value = tmp.Value 
			and tmp2.SubValue = tmp.SubValue and tmp2.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as ffsd_not_at_work, [System], Value, SubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'SID' and Is_Working = 0
			group by [System], Value, SubValue, claim_type
		) tmp3 ON tmp3.[System] = tmp.[System] and tmp3.Value = tmp.Value 
			and tmp3.SubValue = tmp.SubValue and tmp3.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as pid, [System], Value, SubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'PID'
			group by [System], Value, SubValue, claim_type
		) tmp4 ON tmp4.[System] = tmp.[System] and tmp4.Value = tmp.Value 
			and tmp4.SubValue = tmp.SubValue and tmp4.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as totally_unfit, [System], Value, SubValue, claim_type
			from claim_all 
			where Med_Cert_Status = 'TU'
			group by [System], Value, SubValue, claim_type
		) tmp5 ON tmp5.[System] = tmp.[System] and tmp5.Value = tmp.Value 
			and tmp5.SubValue = tmp.SubValue and tmp5.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as therapy_treat, [System], Value, SubValue, claim_type
			from claim_all
			...
			and tmp6.SubValue = tmp.SubValue and tmp6.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as d_d, [System], Value, SubValue, claim_type
			from claim_all 
			where Is_D_D = 1
			group by [System], Value, SubValue, claim_type
		) tmp7 ON tmp7.[System] = tmp.[System] and tmp7.Value = tmp.Value 
			and tmp7.SubValue = tmp.SubValue and tmp7.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as med_only, [System], Value, SubValue, claim_type
			from claim_all 
			where Is_Medical_Only = 1
			group by [System], Value, SubValue, claim_type
		) tmp8 ON tmp8.[System] = tmp.[System] and tmp8.Value = tmp.Value 
			and tmp8.SubValue = tmp.SubValue and tmp8.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as lum_sum_in, [System], Value, SubValue, claim_type
			from claim_all 
			...
			and tmp9.SubValue = tmp.SubValue and tmp9.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN 
		(
			select COUNT(distinct Claim_No) as ncmm_this_week, [System], Value, SubValue, claim_type
			from claim_all 
			...
			and tmp10.SubValue = tmp.SubValue and tmp10.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as ncmm_next_week, [System], Value, SubValue, claim_type
			from claim_all 
			where NCMM_Actions_Next_Week <> ''
				and NCMM_Prepare_Action_Due BETWEEN DATEADD(week, 1, '2016-01-11 23:59') AND DATEADD(week, 3, '2016-01-11 23:59')
			group by [System], Value, SubValue, claim_type
		) tmp11 ON tmp11.[System] = tmp.[System] and tmp11.Value = tmp.Value 
			and tmp11.SubValue = tmp.SubValue and tmp11.claim_type = tmp.Claim_Type
			
		LEFT OUTER JOIN
		(
			select COUNT(distinct Claim_No) as overall, [System], Value, SubValue, claim_type
			from claim_all
			group by [System], Value, SubValue, claim_type
		) tmp12 ON tmp11.[System] = tmp.[System] and tmp12.Value = tmp.Value 
			and tmp12.SubValue = tmp.SubValue and tmp12.claim_type = tmp.Claim_Type
	)
